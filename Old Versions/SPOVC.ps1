...
function Show-FolderSelectionDialog {
    ...
    $libraryRoot = (Get-PnPList -Identity $LibraryName).RootFolder.ServerRelativeUrl

    $allFolders = @($libraryRoot)
    $allFolders += Get-AllPnPSubFolders -ParentFolderUrl $libraryRoot

    $libraryRootPattern = "^" + [regex]::Escape($libraryRoot.TrimEnd('/')) + "/?"
    $relativeFolders = $allFolders | ForEach-Object {
        ($_ -replace $libraryRootPattern, "")
    } | Where-Object { $_ -ne "" }
...
function Get-AllPnPSubFolders {
    param(
        [string]$ParentFolderUrl
    )
    $allFolders = @()
    $subFolders = Get-PnPFolderItem -FolderSiteRelativeUrl $ParentFolderUrl -ItemType Folder -ErrorAction SilentlyContinue
    foreach ($folder in $subFolders) {
        $thisUrl = $folder.ServerRelativeUrl
        $allFolders += $thisUrl
        $allFolders += Get-AllPnPSubFolders -ParentFolderUrl $thisUrl
    }
    return $allFolders
}
